...
const MealDelivery = () => {
    ...
    const fetchMealDeliveries = async () => {
      try {
        const response = await api.get(
          "http://localhost:5000/api/mealdeliveries/"
        ); // Updated endpoint
  ...
  const handleMarkAsDelivered = async (mealId) => {
    try {
      await api.post(`http://localhost:5000/api/mealdeliveries/${mealId}/complete`, {
        deliveryNote,
      }); // Corrected endpoint format with backticks
      ...
      ); // Update local state to reflect delivery status and note
    } catch (err) {
    ...
    <div className="p-6">
      ...
      <div>
        {mealDeliveries.length > 0 ? (
            ...
            .map((meal) => (
              ...
              >
                <h3 className="font-bold text-lg">
                  Meal for{" "}
                  {meal.patientId ? meal.patientId.name : "Unknown Patient"}
                </h3>
                <p>
                  <strong>Room:</strong>{" "}
                  {meal.patientId ? meal.patientId.roomNumber : "N/A"}
                </p>
          ...
          <p>No meal deliveries available.</p>
        )}
      </div>
    </div>
  );
